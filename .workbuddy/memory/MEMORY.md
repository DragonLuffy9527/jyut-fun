...
- **Google Play 逐步操作見 `docs/GooglePlay上架操作手冊.html`**（2026-09-18 查證）：整條路徑**約 3 週起**，14 天封測等待不可壓縮。要點：① **2026 年起 Google 查核測試者真實參與度**，只裝不開會以「參與度不足」駁回，模擬器不算 → 招 14–16 人留緩衝；② **targetSdk 36 自 2026-08-31 強制**，本專案正好達標；③ **AI 生成內容標籤**（2026-04-15 生效）只管**執行時** AI 產出 —— 本 App 語音為預先生成靜態檔、不呼叫模型，**不適用**，商店素材亦非 AI 生成（如實勾「否」）；④ Play App Signing 強制，上傳金鑰遺失**可申請重置**（容錯優於無此機制）；⑤ 目標對象選 **13 歲以上**，勿選兒童以免觸發 Families 政策。
- **`store/listing.md` 的兩個硬限制**：Play 說明欄**不支援 markdown**（`**粗體**` 會原樣顯示），且**簡短說明上限 80 字元**（曾 83 超限被 Console 拒存，已改 78）。改文案後務必重核長度並保持純文字。
